...
The brief for any coding agent working in this repository. It is the canonical
instruction file: `CLAUDE.md` points here, and a harness that reads one file
...
- **Every command works two ways.** `npm run test`, `npm run lint`, `npm run build` and `npm run check:docs` wrap the Nx targets, so a harness that only knows npm needs nothing else. The Nx forms below are equivalent and let you name one project.
- **Run `npm run check:docs` after changing a route or a document.** It fails when a document names a file that is not there, points at a line number, or describes a route the API does not serve.
...
- **Read the ADR before changing what it decided.** `docs/adr/` holds twelve. If your change contradicts one, say so rather than quietly overriding it.
...
```bash
npm test                               # Every project, through Nx
npm run check:docs                     # Documents against the code they describe
...
- `src/auth/` — JWT authentication (`AuthService`, `AuthController`, `password.ts`, the one place a password becomes a hash — the Demo seed uses it too — and `login-attempts.ts`, which throttles failed sign-ins two ways: per source **and** User so a password cannot be guessed, and per source alone so one host cannot work through a list of Users. Only failures count, and a success clears that User's record. The counts live in Redis and fail closed, so both replicas share one budget — ADR-0010). `POST /auth/register` creates a User with a `bcryptjs` hash (see ADR-0003) and returns a token; `POST /auth/login` verifies against that hash and issues a short-lived access token (15 minutes) plus a refresh cookie.
  - `refresh-sessions.ts` — the refresh-token machine (ADR-0009) read whole in one file: issue → rotate → reuse-detect → revoke, opaque tokens hashed at rest with a family id, rotation on every exchange, and family revocation when a rotated token is presented again. `refresh-cookie.ts` names the cookie the token travels in and the one wording every dead-session refusal answers with; `opaque-token.ts` is the mint-and-hash both token machines share
  - `email-tokens.ts` — the one token machine behind password reset and address verification (ADR-0011): single-use in the transaction that consumes them, superseding, with the lifetimes environment-configurable. `POST /auth/forgot-password` is enumeration-safe and revokes every refresh family on reset; unverified Users may still sign in, and the gate is a one-line change
- `src/audit/` — The Platform audit trail (ADR-0012). `audit-events.ts` holds the complete event vocabulary. `AuditService.append(client, event)` writes through the `DatabaseService` or transaction client it is handed; every data mutation passes its own `tx`, so the mutation and event commit together. Authentication and guard events use the best-effort wrapper: an append failure logs under the request id and never changes the response. `pseudonymizeActor(userId)` replaces acting ids with an unlinked random token and records that redaction in the same transaction. The table has no route or page
...
- `src/cache/` — Redis caching (ADR-0005). `CacheService` is the one place the Starter talks to the cache and every method fails open — a Redis that is unreachable costs a request its speed and nothing else. `redis-store.ts` configures the store to fail fast rather than queue. `cache-status.ts` names the `X-Cache: HIT|MISS` header the cached routes answer with, and `CacheService.check()` is what readiness reports about the cache — it observes Redis through the same deadline but neither marks nor honours the failure cooldown, because a probe is not traffic the cooldown protects. The module is `@Global()`
- `src/metrics/` — The opt-in Prometheus endpoint (ADR-0008): `metrics.service.ts` owns the registry and the HTTP duration histogram, `metrics.middleware.ts` records one observation per request on the response's `finish` — the only point where the status the exception filter wrote is final — and `metrics.controller.ts` serves the exposition or refuses with 404. All of it is silent until `METRICS_ENABLED=true`
...
- `src/config/` — App config, guards, pipes, filters:
  - `guards/auth.guard.ts` — JWT bearer token guard, registered globally via `APP_GUARD` (see ADR-0002); attaches `req.user`. Every route requires a token unless it opts out
  - `decorators/public.decorator.ts` — `@Public()` opts a route out of the global guard. The root/health routes, login and registration carry it, and so do the routes whose credential is not an access token: refresh and logout present a refresh cookie, and the three email-flow routes present a token from an email
  - `decorators/current-user.decorator.ts` — `@CurrentUser()` hands a route the claims `auth.guard.ts` attached; `GET /users/me` is what reads the id off it
  ...
  - `guards/api-key.guard.ts` — `x-api-key` header guard; reads `API_KEY` env var; throws `ForbiddenException` on failure. Never stacked on top of the JWT guard
  ...
  - `throttling.ts` — The perimeter throttle (ADR-0010): the NestJS throttler on the global guard over the shared fail-fast Redis connection from `redis-client.ts`, wrapped so a limiter that cannot count answers 503 rather than waving the request through. Registration carries its own tighter ceiling
  ...
  - `logging.ts` — The logging convention (ADR-0008) in one file: pino via `nestjs-pino`, one JSON line per request under a requestId minted by `requestCorrelation` and echoed as `X-Request-Id`, `authorization`/`x-api-key`/`cookie` redacted at the logger, and the machine traffic (probes, scraper) left out of the log on purpose
...
- `LOG_LEVEL` — How much the JSON logs say: one of pino's levels, `info` unless set. A value that is none of them is refused at startup like a required variable
- `AUDIT_RETENTION_MONTHS` — Positive whole calendar months kept by the documented audit partition-drop operation; defaults to `12`. The API validates this value but does not schedule retention
- `METRICS_ENABLED` — The exact string `"true"` serves the Prometheus exposition at `/api/metrics`; unset means off everywhere (ADR-0008)
- `NODE_ENV`, `PORT`
...
- `src/lib/session.ts` — two `httpOnly`, `sameSite=lax` cookies: `aze_session` holding the access token and expiring with it (15 minutes), and `aze_refresh` holding the refresh token (ADR-0009). Browser script cannot read either, which is why every call is made server-side and why no CORS header is involved in the client's own path
- `src/middleware.ts` — redirects to `/login` when neither cookie is present, so a page added later is protected by existing. It never verifies a token — that is the API's job — but it does rotate silently: an expired access cookie with a live refresh cookie is exchanged for a fresh pair before the page renders. It also mints the per-request CSP nonce, which is why the client's `Content-Security-Policy` lives here and not in `next.config.js`
- `src/app/actions.ts` — `login`, `logout` and the email-flow form actions. Credentials never reach browser JavaScript and no token ever leaves the server
...
CORS on the API is `CORS_ORIGIN`, defaulting to `http://localhost:3000`. It matters only for callers that reach the API _from a browser_ — the docs page, another front end — because the client's own pages call it from the Next server.
