...
**Status:** accepted — implemented in #1.

The Starter previously used SQLite, which cannot survive a pod restart or a second replica — a direct contradiction of the Docker and Kubernetes deployment path the Starter intends to ship (#13, #14). Prisma pins the provider in `migration_lock.toml` and generates provider-specific migrations, so "SQLite locally, Postgres in production" would mean two schemas and two migration histories rather than a config flag. We removed SQLite entirely and made Postgres (via `docker-compose`) the only supported database.
